...
class MultiPoseDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    # max_objs: how many targets might be prese nt in one sampe
    ...
    hm_hp = np.zeros((all_num_kps, output_res, output_res), dtype=np.float32) # gaussian hp
    ...
    for k in range(num_objs):
      ...
      if (h > 0 and w > 0) or (rot != 0):
        ...
        if has_kpts:
          for j in range(num_kpts_cls):
            pts[j] = affine_transform(pts[j], trans_output_rot)
            if pts[j, 0] >= 0 and pts[j, 0] < output_res and \
               pts[j, 1] >= 0 and pts[j, 1] < output_res:
              kps[k, (cls_start_idx[cls_id]+j)*2: (cls_start_idx[cls_id]+j)*2 + 2] = pts[j] - ct_int
              kps_mask[k, (cls_start_idx[cls_id]+j)*2: (cls_start_idx[cls_id]+j)*2 + 2] = 1
              pt_int = pts[j].astype(np.int32)
              hp_offset[k * all_num_kps + (cls_start_idx[cls_id]+j)] = pts[j] - pt_int
        ...
        gt_det.append([ct[0] - w / 2, ct[1] - h / 2,
                       ct[0] + w / 2, ct[1] + h / 2, 1] +
                       all_pts.reshape(all_num_kps * 2).tolist() + [cls_id])
    if rot != 0:
      hm = hm * 0 + 0.9999
      reg_mask *= 0
      kps_mask *= 0
