...
class HOLO(data.Dataset):
    ...
    def __init__(self, opt, split):
        ...
        self.data_dir = os.path.join(opt.data_dir, 'heduo-2')
        self.img_dir = os.path.join(self.data_dir, 'splits', opt.holo_split, split)
        self.annot_path = os.path.join(self.data_dir, 'annotations',
                                       'heduo-2-{}-{}.json').format(opt.holo_split, split)
        self.max_objs = 50
        self.class_name = ['__background__', 'Car', 'Truck', 'Engineering_truck', 'Bus', 'Tricycle', 'Rider',
                           'Bicycle', 'Biker', 'Motorcycle', 'Pedestrian', 'Cone', 'Ignore', 'Other']
        self.cat_ids = {1: 0, 2: 0, 3: 0, 4: 0, # vehicle
                        5: 1,
                        6: 2, 8: 2,
                        7: 3, 9: 3,
                        10: 4,
        ...
        print('==> initializing holo {}, {} data.'.format(opt.holo_split, split))
